...
export default function SoumissionDetailPage() {
      ...
      try {
        ...
        if (sData.client_id) {
          try {
            ...
            if (cl.ok) {
              ...
              cEmail = c.email || "";
              cAddress = c.address || "";
              if (!contactIdForMessage && c.lead_id)
                contactIdForMessage = c.lead_id;
            }
          } catch {
            /* ignore */
          }
        ...
        if (contactIdForMessage) {
          const cr = await authedFetch(
            `/api/v1/devlog/leads/${contactIdForMessage}`
          );
          if (cr.ok) {
  ...
  async function syncToQbo(options?: { silent?: boolean }) {
    ...
    try {
      const res = await authedFetch(`/api/v1/devlog/soumissions/${id}/qbo/sync`, {
        method: "POST"
      });
      if (!res.ok) {
  ...
  async function sendForContractorSignature() {
    ...
    try {
      const res = await authedFetch(
        `/api/v1/devlog/soumissions/${id}/send-for-contractor-signature`,
        { method: "POST" }
      );
      if (!res.ok) {
  ...
  async function previewPdf() {
    try {
      ...
      const res = await authedFetch(`/api/v1/devlog/soumissions/${id}/pdf`);
  ...
  async function convertToProject() {
    ...
    try {
      const res = await authedFetch(
        `/api/v1/devlog/soumissions/${id}/convert-to-project`,
        { method: "POST" }
      );
      if (!res.ok) {
      ...
      router.replace(`/dev-logiciel/projets/${project.id}`);
    } catch (err) {
      setSendNotice(
  ...
  async function sendToClient() {
    ...
    setSendNotice(null);
    try {
      const res = await authedFetch(`/api/v1/devlog/soumissions/${id}/send`, {
        method: "POST",
        body: JSON.stringify({
          to,
  ...
  async function deleteSoumission() {
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      router.replace("/dev-logiciel/soumissions");
    } catch {
      setDeleting(false);
  ...
  async function addItem(kind: "service" | "frais" | "rabais" = "service") {
    ...
    try {
      ...
      const res = await authedFetch(`/api/v1/devlog/soumissions/${id}/items`, {
        method: "POST",
        body: JSON.stringify({
  ...
  async function patchItem(item_id: number, patch: Partial<Item>) {
    ...
    try {
      const res = await authedFetch(
        `/api/v1/devlog/soumissions/${id}/items/${item_id}`,
        {
          method: "PATCH",
          ...
          <>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <button
                type="button"
                onClick={previewPdf}
                className="flex items-start gap-3 rounded-xl border border-brand-800 bg-brand-900 p-4 text-left transition hover:border-blue-500"
              >
              ...
              <button
                ...
                className={`flex items-start gap-3 rounded-xl border p-4 text-left transition ${
                  isQboSynced
                    ...
                    : "border-brand-800 bg-brand-900 hover:border-blue-500"
                } disabled:opacity-60`}
              >
                {qboBusy ? (
                  ...
                  <RefreshCw
                    className={`mt-0.5 h-5 w-5 flex-shrink-0 ${
                      isQboSynced ? "text-emerald-400" : "text-blue-400"
                    }`}
                  />
                )}
                <div>
            ...
            {s.status === "accepted" ? (
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <p className="w-full text-xs text-white/50">
                  ...
                  <span className="text-blue-300"> brouillon</span> dans la
                  facturation.
                </p>
              </div>
            ) : null}
                  ...
                  {s.contractor_signed_name ? (
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-xl bg-blue-500 px-5 py-3 font-semibold text-white transition hover:bg-blue-400 text-sm disabled:opacity-50"
                      >
                        {sendingForSignature ? (
                          <>
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ...
                <div className="rounded-xl border border-brand-800 bg-brand-900 p-5">
                  ...
                  <dl className="mt-4 space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between border-t border-brand-800 pt-3">
                      ...
                      <dd className="text-lg font-bold text-blue-400">{fmtMoney(total)}</dd>
                    </div>
                  </dl>
                    ...
                    <div>
                      ...
                      <dd className="mt-0.5 text-white">
                        {s.lead_id ? (
                          <Link
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            href={`/dev-logiciel/leads/${s.lead_id}` as any}
                            className="text-blue-400 hover:text-blue-300"
                          >
                            Fiche prospect #{s.lead_id}
                          </Link>
                        ) : (
    ...
    <div className="mt-3 space-y-2">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <a
          ...
          className="inline-flex items-center gap-1 rounded-md border border-brand-800 bg-brand-900 px-2 py-1 text-white/70 hover:border-blue-500 hover:text-white"
        >
              ...
              {filtered.map((t) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => apply(t)}
                    disabled={busy === t.id}
                    className="rounded-lg bg-blue-500 px-3 py-2 text-xs font-bold text-brand-950 disabled:opacity-60"
                  >
                    {busy === t.id ? (
