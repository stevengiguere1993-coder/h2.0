...
function AppUsersTopbar({ rightSlot }: { rightSlot: React.ReactNode }) {
...
export function UsersManager({
  ...
  const [allImmeubles, setAllImmeubles] = useState<ImmeubleMini[]>([]);
  const [savingImmeubles, setSavingImmeubles] = useState(false);
  const [dirtyImmIds, setDirtyImmIds] = useState<Set<number> | null>(null);
  ...
  const loadImmeubleAssignments = useCallback(async (userId: number) => {
    setSavingImmeubles(false);
    setDirtyImmIds(null);
    try {
      const [aRes, iRes] = await Promise.all([
        authedFetch(`/api/v1/users/${userId}/immeubles`),
        authedFetch("/api/v1/immobilier/immeubles?only_active=false")
      ]);
      if (aRes.ok) setImmAssignments((await aRes.json()) as ImmeubleMini[]);
      if (iRes.ok) setAllImmeubles((await iRes.json()) as ImmeubleMini[]);
    } catch {
      /* volet immobilier peut être indisponible : on ignore en silence */
    }
  }, []);
  ...
  useEffect(() => {
    if (selected != null) {
      void loadAssignments(selected);
      void loadImmeubleAssignments(selected);
    } else {
      setAssignments([]);
      setDirtyIds(null);
      setImmAssignments([]);
  ...
  }, [selected, loadAssignments, loadImmeubleAssignments]);
  ...
  function toggleImmeuble(immeubleId: number) {
    setDirtyImmIds((prev) => {
      const next = new Set(prev ?? new Set(immAssignments.map((a) => a.id)));
  ...
  async function saveImmeubles() {
    if (selected == null || dirtyImmIds == null) return;
    setSavingImmeubles(true);
    try {
      const res = await authedFetch(
        `/api/v1/users/${selected}/immeubles`,
        {
          method: "PUT",
          body: JSON.stringify({ immeuble_ids: Array.from(dirtyImmIds) })
        }
      );
      if (!res.ok) throw new Error();
      await loadImmeubleAssignments(selected);
    } catch {
      setError("Sauvegarde des immeubles échouée.");
    } finally {
      setSavingImmeubles(false);
    }
  }

  const currentImmIds = useMemo(
    () => new Set(immAssignments.map((a) => a.id)),
    [immAssignments]
  );
  const effectiveImmIds = dirtyImmIds ?? currentImmIds;
  const hasImmChanges =
    dirtyImmIds != null &&
    (dirtyImmIds.size !== currentImmIds.size ||
      Array.from(dirtyImmIds).some((id) => !currentImmIds.has(id)));
  ...
  return (
    <>
      {variant === "immobilier" ? (
        ...
        <AppUsersTopbar rightSlot={addBtn} />
      )}
            ...
            <section className="rounded-xl border border-brand-800 bg-brand-900">
              {selectedUser ? (
                <>
                  ...
                  {selectedUser.role === "employee" &&
                  (selectedUser.volets || []).includes("immobilier") ? (
                    <div className="border-t border-brand-800 p-4">
                      <div className="flex items-center justify-between">
                        ...
                        {hasImmChanges ? (
                          <button
                            type="button"
                            onClick={saveImmeubles}
                            disabled={savingImmeubles}
                            className="btn-accent text-xs disabled:opacity-60"
                          >
                            {savingImmeubles ? (
                              <Loader2 className="h-3.5 w-3.5 animate-spin" />
                      ...
                      {allImmeubles.length === 0 ? (
                        ...
                        <ul className="mt-3 max-h-80 space-y-1 overflow-y-auto">
                          {allImmeubles.map((im) => {
                            const checked = effectiveImmIds.has(im.id);
                            return (
                              <li key={im.id}>
                                <label
                                  className={`flex cursor-pointer items-center gap-2 rounded-md border border-brand-800 px-3 py-2 text-sm transition hover:border-accent-500 ${
