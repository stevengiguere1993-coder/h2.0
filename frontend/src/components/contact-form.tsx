...
// Loads the Google Maps Places library once per page even if the
// ContactForm is mounted several times.
let _placesLoader: Promise<void> | null = null;
function loadGooglePlaces(apiKey: string, lang: string): Promise<void> {
  ...
  _placesLoader = new Promise((resolve, reject) => {
    ...
    script.src = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(
      apiKey
    )}&libraries=places&language=${langParam}&region=CA&loading=async`;
    script.async = true;
    script.defer = true;
...
export function ContactForm({ source }: { source?: string }) {
  ...
  useEffect(() => {
    ...
    loadGooglePlaces(apiKey, locale)
      ...
      .catch(() => {
        /* silent — plain text input remains usable */
      });
